---
title: "Bachelor's Degree by Composition"
...
garden_harvest %>%
  ...
  group_by(variety) %>%
  summarise(var_weight = sum(weight)) %>% 
  mutate(n = round(var_weight/sum(var_weight)*99.00990099)) %>%
  arrange(desc(n)) %>%
  mutate(variety = str_to_title(variety),
         variety = fct_rev(variety),
         variety = fct_reorder(variety, n)) %>%
  ...
  scale_fill_manual(
    name = NULL,
    values = c("grey22", "darkslategrey", "salmon4", "tomato3", "tomato", "salmon", 
               "khaki2", "darkgoldenrod1", "orangered1", "firebrick3", "firebrick4", 
               "violetred4")) +
  labs(title = "Distribution of Tomato Variety by Weight (g) Produced", 
       subtitle = "Total weight of 158,231 g produced.",
       fill = "") +
  theme(
